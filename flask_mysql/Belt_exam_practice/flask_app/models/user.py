...
class User:
    ...
    #     query = "SELECT first_name FROM users LEFT JOIN comments ON users.id = comments.user_id WHERE comments.user_id= 7;"
    #     result = connectToMySQL(DATABASE).query_db(query, data)
    #     print(result)
    ...
    def get_one_with_comments(cls, data):
        query = "SELECT * FROM users LEFT JOIN comments ON users.id = comments.user_id WHERE users.id = %(id)s ;"
        ...
        for result in results:
            ...
            user.comments.append(Comment(comment_dict))
        print(user)
        return user
    ...
    # @staticmethod
    # def validate_thought(user:dict):
    #     is_valid = True
    #     if len(user['text']) < 4:
    #         is_valid = False
    ...
    # @classmethod
    # def update(cls, data):
    #     query = "UPDATE comments SET first_name = %(first_name)s, last_name = %(last_name)s, email = %(email)s WHERE id = %(id)s ;"
    #     results = connectToMySQL(DATABASE).query_db(query, data)